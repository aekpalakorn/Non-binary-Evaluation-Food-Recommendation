...
class FPMC(object):
    ...
    def init_model(self, std=0.01):
        self.VUI = np.random.normal(0, std, size=(self.n_user, self.n_factor)).astype(np.float64)
        self.VIU = np.random.normal(0, std, size=(self.n_item, self.n_factor)).astype(np.float64)
        self.VIL = np.random.normal(0, std, size=(self.n_item, self.n_factor)).astype(np.float64)
        self.VLI = np.random.normal(0, std, size=(self.n_item, self.n_factor)).astype(np.float64)
    ...
    def learn_epoch(self, data_3_list, neg_batch_size):
        VUI, VIU, VLI, VIL = learn_epoch_jit(data_3_list[0], data_3_list[1], data_3_list[2], neg_batch_size,
                                             np.array(list(self.item_set), dtype=np.int32), 
...
def learn_epoch_jit(u_list, i_list, b_tm1_list, neg_batch_size, item_set, VUI, VIU, VLI, VIL, learn_rate, regular):
    for iter_idx in range(len(u_list)):
        d_idx = np.random.randint(0, len(u_list), dtype='int32')
        u = u_list[d_idx]
        i = i_list[d_idx]
        ...
        for j in j_list:
            ...
            eta = np.zeros(VLI.shape[1]).astype(np.float64)
            for l in b_tm1:
                eta += VLI[l]
            eta = eta / len(b_tm1)
            ...
            VLI_updates = np.zeros((len(b_tm1), VLI.shape[1])).astype(np.float64)
...
def compute_x_batch_jit(u, b_tm1, VUI_m_VIU, VIL_m_VLI):
    former = VUI_m_VIU[u]
    latter = np.zeros(VIL_m_VLI.shape[0]).astype(np.float64)
...
def data_to_3_list(data_list):
    ...
    b_tm1_list = np.array(b_tm1_list, dtype='int32')
    return np.array(u_list, dtype='int32'), np.array(i_list, dtype='int32'), b_tm1_list
