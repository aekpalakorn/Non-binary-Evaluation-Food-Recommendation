...
from src import log_utils as log
from src.io import load_pickle


def get_dataset(dataset_name, data_dir=DATA_DIR, logging=True, on_val=False):
    """
    ...
    cols = ['UID', 'PID', 'cnt']
    if on_val:
        df_train = pd.read_csv(os.path.join(data_dir, dataset_name, 'previous_train.csv'))[cols]
        df_val = pd.read_csv(os.path.join(data_dir, dataset_name, 'last_train.csv'))[cols]
        df_test = pd.read_csv(os.path.join(data_dir, dataset_name, 'validation.csv'))[cols]
    else:
...
# def get_decay_dataset(dataset_name, decay,  data_dir=DATA_DIR, is_eval=True, logging=True):
#     """
#     Returns train, val and test data for a dataset. They are in COO form, as required by the Global, Personal, Mixture,
#     NMF and LDA models.
...
#     print(df_train.TS.unique())
#     df_val = pd.read_csv(os.path.join(data_dir, dataset_name, 'validation.csv'))[cols]
#     print(df_val.head(2))
#     print(df_val.TS.unique())
#     df_test = pd.read_csv(os.path.join(data_dir, dataset_name, 'test.csv'))[cols]
#     for col in cols:
...
def get_decay_dataset(dataset, decay, is_eval=True, data_dir=DATA_DIR):
    cols = ['UID', 'PID', 'cnt']
    df_val = pd.read_csv(os.path.join(data_dir, dataset, 'validation.csv'))[cols]
    df_test = pd.read_csv(os.path.join(data_dir, dataset, 'test.csv'))[cols]

    df_train = pd.read_csv(os.path.join(data_dir, dataset, 'train.csv'))
    # TS later on top
    df_train = df_train.sort_values(['TS'], ascending=False)
    grouped = df_train[['UID', 'TS']].drop_duplicates().groupby('UID', as_index=False)
    result = grouped.apply(lambda x: x.reset_index(drop=True)).reset_index().drop('level_0', axis=1)
    new_df = pd.merge(df_train, result, how='left', left_on=['UID', 'TS'], right_on=['UID', 'TS'])
    if is_eval: # last basket in train apply decay
        new_df['decay_cnt'] = decay ** (new_df['level_1'] + 1) * new_df['cnt']    
    else: # last basket in train not apply decay
        new_df['decay_cnt'] = decay ** (new_df['level_1']) * new_df['cnt']
    new_df = new_df[['UID', 'PID', 'decay_cnt']]
    return new_df.values, df_val.values, df_test.values
...
def get_hpf_dataset(dataset_name, data_dir=DATA_DIR, logging=True):
    """
...
def get_fpmc_dataset(dataset, data_dir=DATA_DIR, is_eval=True):
    # :param is_eval: Boolean, that defines if this data will be used for evaluation or training. The difference is that
    # when evaluating, the validation data is added to the training, otherwise it is not.
...
def get_fpmc_test(dataset, data_dir=DATA_DIR, is_eval=True):
    # :param is_eval: Boolean, that defines if this data will be used for evaluation or training. The difference is that
    # when evaluating, the validation data is added to the training, otherwise it is not.
    if is_eval:
        file_name = 'test.csv'
